...
namespace Discount.API.Repository
{
    public class CouponRepository : ICouponRepository
    {
        ...
        {
            var connection = new NpgsqlConnection(_configuration.GetConnectionString("DiscountDB"));
            var affected = await connection.ExecuteAsync("UPDATE Coupon SET ProductId=@ProductId, ProductName=@ProductName, Description=@Description, Amount=@Amount WHERE ProductId=@ProductId",
                new
                {
                    ProductId = coupon.ProductId,
                    ProductName = coupon.ProductName,
